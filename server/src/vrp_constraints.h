...
public:
    ...
    void setup(bool enforce_soft, const std::vector<Employee>& emps,
        ...
        // Vehicle preference constraints - REMOVED (kept as soft)
        ...
        // Vehicle MODE compatibility - HARD domain restriction (Phase 3B)
        // P1 employees and premium-preference employees CANNOT ride 2-wheelers
        int mode_pruned = 0;
        for (int i = 0; i < n_emp; i++) {
            for (int v = 0; v < n_veh; v++) {
                if (!is_mode_compatible(emps[i].priority, emps[i].vehicle_pref, virt_vehs[v].vehicle_mode)) {
        ...
        if (mode_pruned > 0) {
            std::cout << "  Mode compatibility: pruned " << mode_pruned << " employee-vehicle pairs" << std::endl;
        }
        ...
        for (int i = 0; i < n_emp; i++) {
            for (int j = i+1; j < n_emp; j++) {
                ...
                // Assume configurable minimum for pickup + travel to office
                const int min_trip_time = g_config.min_trip_time_minutes;
