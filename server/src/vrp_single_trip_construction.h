            ...
            if (best_v >= 0) {
                ...
                if (is_infeasible[e]) {
                    ...
                    for (int v = n_veh - 1; v >= 0; v--) {
                        ...
                        // Vehicle preference is CRITICAL - use config penalty
                        double pref_penalty = pref_violation ? g_config.infeasible_pref_penalty : 0.0;
                        ...
                        double score = pref_penalty
                                     - trip_idx * g_config.infeasible_trip_idx_weight   // Strongly prefer later trip indices
                                     + (has_earlier ? -g_config.infeasible_earlier_trip_bonus : 0.0)  // Prefer vehicles with earlier trips used
                                     + lateness * g_config.infeasible_lateness_coeff;      // Slight preference for less lateness (but not dominant)
