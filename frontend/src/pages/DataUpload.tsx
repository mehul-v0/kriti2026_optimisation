...
export default function DataUpload() {
  ...
  // Load existing uploaded data on mount
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        if (parsed.employees && parsed.vehicles) {
          ...
          // Create a virtual file object to represent the uploaded state
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    if (files && files[0]) {
      // Clear old state before processing new file
      setParsedData(null);
      setValidationError(null);
  ...
  return (
    <div className="min-h-screen p-3">
      ...
      <section className="mb-3">
        <div className="bg-dark-800/60 backdrop-blur-xl rounded-2xl border border-gray/10 p-8 shadow-float">
          ...
          >
            <div className="text-center">
              <Upload className={`w-12 h-12 mx-auto mb-4 mt-8 transition-colors ${
                dragActive ? 'text-primary-bright' : 'text-gray/60'
              }`} />
              ...
              <div className="flex items-center justify-center gap-3 mb-8">
                {/* Main Select Button */}
                <label className="bg-primary hover:bg-primary-light text-dark font-bold px-8 text-base rounded-xl cursor-pointer transition-all inline-flex items-center gap-2 w-80 justify-center h-14">
                  <input
                    type="file"
                    accept=".xlsx,.xls"
                ...
                <div className="relative group">
                  <button 
                    className="w-14 h-14 bg-primary-muted/10 hover:bg-primary-muted/20 rounded-xl flex items-center justify-center transition-all"
                    onClick={() => {/* TODO: Implement Google Drive */}}
                  >
                    <Cloud className="w-7 h-7 text-primary-muted" />
                  </button>
                ...
                <div className="relative group">
                  ...
                  >
                    <HardDrive className="w-7 h-7 text-primary-muted" />
                  </button>
                  <div className="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 bg-dark-700 text-white text-xs px-3 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
      ...
      {(uploadedFile || isValidating || validationError || parsedData) && (
        <section className="mb-3">
          <div className="bg-dark-800/60 backdrop-blur-xl rounded-2xl border border-gray/10 p-6 space-y-4 shadow-float">
            ...
            {uploadedFile && (
              <motion.div
                ...
                className="bg-dark-700/50 rounded-xl p-4 border border-gray/5 shadow-float"
              >
            ...
            {/* Validation Loading - Enhanced with themed spinner */}
            {isValidating && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-dark-700/50 rounded-xl p-8 text-center border border-gray/5 backdrop-blur-sm"
              >
                <LoadingSpinner 
                  size="lg" 
                  text="Parsing and validating your data"
                  className="py-2"
                />
              </motion.div>
            )}
            ...
            {parsedData && !validationError && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="bg-primary/10 border-primary/30 rounded-xl p-4 border"
              >
                <div className="flex items-start gap-3">
                  <Check className="w-5 h-5 text-primary-bright flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="font-medium text-primary-bright mb-1">File parsed successfully!</p>
          ...
          <div className="bg-dark-800/60 backdrop-blur-xl rounded-2xl border border-gray/10 p-6 shadow-float">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                className="bg-primary hover:bg-primary-light text-dark font-bold px-6 py-3 rounded-lg transition-all hover:scale-105 inline-flex items-center gap-2"
              >
