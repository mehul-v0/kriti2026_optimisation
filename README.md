...
- **C++ Solver fails to compile**: Ensure you have installed MinGW and added it to your system PATH. On Linux, ensure `build-essential` is installed.
- **Port 5000 / 5173 is already in use**: If you get a port conflict, you can change the backend port in `server/app.py` or the frontend port by running `npm run dev -- --port <new_port>`.
- **Frontend can't connect to backend**: Verify that your backend is running on `http://127.0.0.1:5000`. If you changed the backend URL, ensure `vite.config.ts` proxy settings are updated.

---

## Contributing

We welcome contributions to improve the VRP Optimization platform!
To contribute:
